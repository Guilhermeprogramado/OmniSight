...
describe("token-bucket async functions", () => {
  ...
  describe("checkTokenBucketLimit", () => {
    ...
    it("does not throw when the monthly bucket is exhausted for free inference", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();
      ...
      // Peek + a rate-0 deduction: free inference never consumes the bucket.
      expect(mockLimitFn).toHaveBeenCalledTimes(2);
    });

    it("does not draw from extra usage when inference is free", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();
      ...
      expect(mockDeductFromBalance).not.toHaveBeenCalled();
      expect(result.pointsDeducted).toBe(0);
      expect(result.extraUsagePointsDeducted).toBeUndefined();
      expect(mockLimitFn).toHaveBeenCalledTimes(2);
    });

    it("keeps rate-0 peeks and deducts for free inference even with chargeAllUsage", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();
    ...
    it("does not surface monthly cap failures for free inference", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();
    ...
    it("does not surface insufficient funds failures for free inference", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();
  ...
  describe("deductUsage", () => {
    it("does not deduct any points when token usage is free", async () => {
      const { deductUsage } = getIsolatedModule();

      const result = await deductUsage("user-123", "pro", 1000, 1200, 500);
    ...
    it("should use extra usage when bucket depleted", async () => {
      ...
      // Provider cost ($0.005 = 75 billable points) is the only non-zero
      // cost; the token estimate is free.
    ...
    it("charges provider cost as additional when token estimates are free", async () => {
      const { deductUsage, billableCostDollarsToPoints } =
        getIsolatedModule();

      // Token estimate for 10000 input is 0 billable points (free inference).
      const estimatedInputTokens = 10000;
      const estimatedCost = 0;

      // Actual provider cost: $0.002 = 30 billable points
      const providerCostDollars = 0.002;
      const actualCostPoints =
      ...
      // The provider cost difference (30 - 0) is charged to the bucket.
      expect(mockLimitFn).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({ rate: actualCostPoints }),
      );
      // No refund happens because the token estimate was already zero.
    ...
    it("performs no token-based refund when estimated and actual costs are free", async () => {
      const { deductUsage } = getIsolatedModule();

      // Estimate: 10000 input tokens = 0 billable points (free inference).
      const estimatedInputTokens = 10000;

      // Actual: 2000 input + 500 output = 0 billable points (free inference).
      ...
      // Nothing to refund and nothing to charge.
      expect(mockHincrbyFn).not.toHaveBeenCalled();
      expect(mockLimitFn).not.toHaveBeenCalled();
      expect(result).toEqual({
        includedPointsDeducted: 0,
    ...
    it("should not refund or charge when actual cost equals estimated", async () => {
      ...
      // Estimate: 1000 input tokens = 0 billable points (free inference).
      const estimatedInputTokens = 1000;

      // Actual token-based cost also computes to 0, so nothing moves.
      await deductUsage(
        "user-123",
        "pro",
        estimatedInputTokens,
    ...
    it("should use served fallback model pricing for actual token cost when provider cost is absent", async () => {
      ...
      const servedModel = "model-glm-5.2-free";
      ...
      expect(expectedAdditional).toBe(0);
      // Token estimates are free, so no additional limiter deduction occurs.
      expect(mockLimitFn).not.toHaveBeenCalled();
      expect(result).toEqual({
    ...
    it("should prefer raw provider cost over served fallback token pricing", async () => {
      ...
      expect(expectedAdditional).toBe(6_300);
      expect(mockLimitFn).toHaveBeenCalledWith(
        expect.any(String),
  ...
  describe("deductUsage - split deduction (peek-then-deduct)", () => {
    ...
    it("should deduct overflow from extra usage when bucket has insufficient balance", async () => {
      ...
      // Estimated 1000 input = 0 points (free), actual provider cost = $0.005 = 75 billable points.
      // Difference = 75 - 0 = 75 additional needed.
      // Bucket has 10, so fromBucket=10, fromExtraUsage=65.
      const result = await deductUsage(
        "user-123",
        "pro",
        1000,
      ...
      // Should deduct the overflow (65) from extra usage
      expect(mockDeductFromBalance).toHaveBeenCalledWith(
        "user-123",
        65,
        undefined,
  ...
  describe("concurrent deduction safety", () => {
    it("allows concurrent free-inference checks to settle without billing or failure", async () => {
      const { checkTokenBucketLimit } = getIsolatedModule();

      // Simulate two concurrent requests seeing the same bucket state.
      // Free inference peeks and deducts at rate 0, so both succeed.
      let callCount = 0;
      mockLimitFn.mockImplementation(
        async (_key: string, opts: { rate: number }) => {
          callCount++;
          // Every call is a rate-0 peek/deduct for free inference.
          expect(opts.rate).toBe(0);
          return {
            success: true,
            remaining: 8,
            reset: Date.now() + 3600000,
      ...
      // Each check performs a peek and a rate-0 deduct.
  ...
  describe("end-to-end scenarios", () => {
    it("typical conversation flow: check -> deduct -> complete", async () => {
      ...
      // Free inference never deducts points upfront.
      expect(rateLimitInfo.pointsDeducted).toBe(0);

      await deductUsage("user-123", "pro", 2000, 2500, 800);

      // Only the check's peek + rate-0 deduct hit the limiter; the free
      // reconciliation performs no additional calls.
      expect(mockLimitFn.mock.calls.length).toBe(2);
      expect(mockDeductFromBalance).not.toHaveBeenCalled();
    });

    it("failed request flow: check -> error -> refund (nothing to refund)", async () => {
      ...
      // Free inference deducted nothing upfront, so no Redis refund runs.

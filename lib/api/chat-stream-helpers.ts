...
// Free-tier routes must never fall back to a paid model on a no-credit
// account, so their chain stays on free Nemotron/GLM provider slugs.
const FREE_TEXT_FALLBACK_CHAIN = [
  "model-glm-5.2-free",
] as const satisfies readonly ModelName[];

// OmniSight Pro uses Grok 4.5 for every request. GLM 5.2 remains its first
...
const MODEL_FALLBACK_CHAIN: Partial<Record<ModelName, readonly ModelName[]>> = {
  "ask-model-free": FREE_TEXT_FALLBACK_CHAIN,
  "agent-model-free": FREE_TEXT_FALLBACK_CHAIN,
...
export function getRetryFallbackModel(
  ...
  if (modelName === "ask-model-free" || modelName === "agent-model-free") {
    return "model-glm-5.2-free";
  }
...
// Every route runs on free provider slugs, so the content-filter retry pool
// only contains free models. If the sole different free model was already
// served, there is no paid model to fall back on and the request errors.
const FREE_CONTENT_FILTER_RETRY_CANDIDATES = [
  "model-glm-5.2-free",
] as const satisfies readonly ModelName[];
...
export function getContentFilterRetryModel(
  ...
  const candidates: readonly ModelName[] = [
    preferredFallback,
    ...FREE_CONTENT_FILTER_RETRY_CANDIDATES,
  ];
...
export function getFallbackSlugs(
  ...
  return (
    fallbackKeys
      ...
      // Multiple chain keys can resolve to the same free slug; dedupe so the
      // `models` list stays clean.
...
const OPENROUTER_RESPONSE_MODEL_COST_KEYS: Record<string, string> = {
  ...
  "nvidia/nemotron-3-super-120b-a12b:free": "ask-model-free",
  // NVIDIA Build serves the same free Nemotron without the `:free` suffix.
  "nvidia/nemotron-3-super-120b-a12b": "ask-model-free",
  ...
  "z-ai/glm-5.2:free": "model-glm-5.2-free",
...
/**
 * Free-tier agent mode is locked until a Stripe payment is confirmed: free
 * users get no Agent tools at all. Allowlisted dev emails resolve upstream to
 * a paid-tier subscription (see resolveDevSubscriptionTier), so they keep
 * Agent access. This gate covers both the chat-handler and the agent-long
 * trigger pipeline.
 */
export function assertFreeAgentGates(args: {
  ...
  if (args.mode === "agent" && args.subscription === "free") {
    throw new ChatSDKError(
      "forbidden:chat",
      "Agent mode requires a paid subscription. Use Ask mode or upgrade to unlock Agent tools.",
    );
  }
}

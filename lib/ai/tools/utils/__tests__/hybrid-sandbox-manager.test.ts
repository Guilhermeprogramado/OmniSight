...
describe("HybridSandboxManager prompt-time fallback", () => {
  ...
  it("records a cloud fallback for free users without a local connection", async () => {
    mockConvexQuery.mockResolvedValue([]);
    ...
    const fallback = manager.consumeFallbackInfo();
    expect(fallback).not.toBeNull();
    expect(fallback?.actualSandbox).toBe("e2b");
  });
  ...
  it("blocks cloud fallback whenever a local sandbox was selected", () => {
    ...
    expect((error as Error & { cause?: unknown }).cause).toContain(
      "OmniSight did not switch this run to Cloud",
    );
  });

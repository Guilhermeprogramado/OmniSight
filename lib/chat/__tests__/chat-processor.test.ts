...
describe("selectModel", () => {
  ...
  it("keeps OmniSight Pro on Grok 4.5 without an experiment assignment", () => {
  ...
  describe("tier override for ask mode (paid users)", () => {
    it("should map OmniSight Pro to Grok 4.5 for text-only ask mode", () => {
      expect(selectModel("ask", "ultra", "hackerai-pro")).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should map OmniSight Pro to Grok 4.5 for team users", () => {
      expect(selectModel("ask", "team", "hackerai-pro")).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should keep OmniSight Pro on Grok 4.5 when an image is attached", () => {
      expect(selectModel("ask", "pro", "hackerai-pro", true, false)).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should keep OmniSight Pro on Grok 4.5 when a PDF is attached", () => {
      expect(selectModel("ask", "pro", "hackerai-pro", false, true)).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should map OmniSight Standard to DeepSeek V4 Pro when no image/PDF", () => {
      expect(selectModel("ask", "pro", "hackerai-standard")).toBe(
        "model-deepseek-v4-pro",
      );
    });

    it("should promote OmniSight Standard to Grok 4.5 when an image is attached", () => {
      expect(selectModel("ask", "pro", "hackerai-standard", true, false)).toBe(
        "model-grok-4.5",
      );
    });

    it("should promote OmniSight Standard to Grok 4.5 when a PDF is attached", () => {
      expect(selectModel("ask", "pro", "hackerai-standard", false, true)).toBe(
        "model-grok-4.5",
      );
    });

    it("should prefer Grok 4.5 for OmniSight Standard when image and PDF are both attached", () => {
      expect(selectModel("ask", "pro", "hackerai-standard", true, true)).toBe(
        "model-grok-4.5",
      );
    });

    it("should map OmniSight Max to its Kimi K3 compatibility route for Ultra", () => {
      expect(selectModel("ask", "ultra", "hackerai-max")).toBe(
        "model-opus-4.6",
      );
    });

    it("should downgrade OmniSight Max to Pro outside Ultra", () => {
    ...
    it("should map OmniSight Max to its Kimi K3 compatibility route for paid users with extra usage", () => {
      expect(
  ...
  describe("tier override in agent mode", () => {
    it("should map OmniSight Standard to DeepSeek V4 Pro for text-only agent mode", () => {
      expect(selectModel("agent", "pro", "hackerai-standard")).toBe(
        "model-deepseek-v4-pro",
      );
    });

    it("should route OmniSight Standard to Grok 4.5 when an image is attached", () => {
      expect(
        selectModel("agent", "pro", "hackerai-standard", true, false),
      ).toBe("model-grok-4.5");
    });

    it("should route OmniSight Standard to Grok 4.5 when a PDF is attached", () => {
      expect(
        selectModel("agent", "pro", "hackerai-standard", false, true),
      ).toBe("model-grok-4.5");
    });

    it("should map OmniSight Pro to Grok 4.5 in text-only agent mode", () => {
      expect(selectModel("agent", "pro", "hackerai-pro")).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should route OmniSight Pro to Grok 4.5 in agent mode when an image is attached", () => {
      expect(selectModel("agent", "pro", "hackerai-pro", true, false)).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should route OmniSight Pro to Grok 4.5 in agent mode when a PDF is attached", () => {
      expect(selectModel("agent", "pro", "hackerai-pro", false, true)).toBe(
        "model-grok-4.5-pro",
      );
    });

    it("should map OmniSight Max to its Kimi K3 compatibility route in agent mode for Ultra", () => {
      expect(selectModel("agent", "ultra", "hackerai-max")).toBe(
        "model-opus-4.6",
      );
    });

    it("should downgrade OmniSight Max to Pro in agent mode outside Ultra", () => {
    ...
    it("should map OmniSight Max to its Kimi K3 compatibility route in agent mode for paid users with extra usage", () => {
      expect(

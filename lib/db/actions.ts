...
// before they must upgrade. Mirrors FREE_ASK_REPORT_LIMIT in convex/chats.ts.
...
export async function saveChat({
  ...
  const mutationArgs = {
    ...
    title,
    ...(projectId ? { projectId } : {}),
    ...(countAsAskReport ? { countAsAskReport: true } : {}),
  };
  ...
  try {
    ...
    const dbErrorData = getErrorData(error);
    if (hasDatabaseErrorCode(dbErrorData, "FREE_ASK_REPORT_LIMIT")) {
      throw new ChatSDKError(
        "forbidden:chat",
        "You've reached the free limit of Ask reports. Upgrade to keep creating Ask reports.",
        {
          db_operation: "chats.saveChat",
          chat_id: id,
          user_id: userId,
          capReason: "free_ask_reports_exhausted",

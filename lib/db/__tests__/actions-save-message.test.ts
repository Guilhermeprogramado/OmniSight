...
describe("saveChat", () => {
  ...
  it("maps the free Ask report limit Convex error to an upgradeable chat error", async () => {
    const { saveChat, mockMutation } = await loadSaveMessageWithMocks();
    const limitError = new Error(
      "Free Ask reports are limited to 1. Upgrade to create more Ask reports.",
    ) as Error & { data?: unknown };
    limitError.name = "ConvexError";
    limitError.data = {
      code: "FREE_ASK_REPORT_LIMIT",
      message: "Free Ask reports are limited to 1.",
      operation: "chats.saveChat",
      limit: 1,
    };
    mockMutation.mockRejectedValue(limitError as never);

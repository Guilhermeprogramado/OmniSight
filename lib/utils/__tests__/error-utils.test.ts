...
import {
  ...
  isProviderDailyFreeModelQuotaError,
...
describe("provider error classification", () => {
  ...
  it("detects OpenRouter daily free-model quota exhaustion as a non-generic error", () => {
    const err = apiCallError({
      statusCode: 429,
      data: {
        error: {
    ...
    expect(isProviderDailyFreeModelQuotaError(err)).toBe(true);
    expect(getUserFriendlyProviderError(err)).toContain(
      "daily free-model request quota",
    );
    expect(getUserFriendlyProviderError(err)).not.toContain(
      "temporarily rate limited",
    );
  });

  it("detects free-model quota exhaustion through RetryError attempt chains", () => {
    const attempt = apiCallError({
      statusCode: 429,
      data: {
        error: {
          message:
    ...
    const err = retryError([attempt, attempt, attempt]);
  ...
  it("does not treat ordinary 429 rate limits as free-model quota exhaustion", () => {
    const err = apiCallError({
      statusCode: 429,
      data: {
        error: { message: "Rate limit exceeded: standard per-minute limit" },
      },
    });

    expect(isProviderDailyFreeModelQuotaError(err)).toBe(false);
    expect(getUserFriendlyProviderError(err)).toContain(
      "temporarily rate limited",
    );
  });

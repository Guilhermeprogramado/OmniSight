...
const OPENROUTER_DAILY_FREE_QUOTA_PATTERN =
  /free\s*models?\s*[-_\s]?\s*per\s*[-_\s]?\s*day|free\s*model\s+requests?\s+per\s+day|unlock\s+1000\s+free\s+model/i;

export const isProviderDailyFreeModelQuotaError = (
  error: unknown,
): boolean =>
  collectErrorSources(error)
    .flatMap(sourceToSearchableStrings)
    .some((text) => OPENROUTER_DAILY_FREE_QUOTA_PATTERN.test(text));
...
export const getUserFriendlyProviderError = (error: unknown): string => {
  ...
  if (isProviderDailyFreeModelQuotaError(error)) {
    return "Your daily free-model request quota was reached. Free models reset their limit each day around midnight UTC — you can retry shortly, or add credits to this account's OpenRouter key to unlock more free requests per day.";
  }

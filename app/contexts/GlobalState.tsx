...
interface GlobalStateType {
  ...
  // Cumulative count of Ask-mode chats created on the free plan (null while
  // unauthenticated/non-free; feeds the 2-report free Ask gate).
  freeAskReportCount: number | null;
...
const GlobalStateProviderInner: React.FC<GlobalStateProviderProps> = ({
  ...
  const subscriptionFromEntitlements = useMemo<SubscriptionTier | null>(() => {
    if (isDevEmail((user as { email?: unknown } | null)?.email as
      | string
      | undefined)) {
      return "ultra";
    }
  ...
  // Cumulative count of Ask-mode chats created on the free plan. Only tracked
  // while the user is free; feeds the 2-report free Ask gate.
  const freeAskReportCount = useQuery(
    api.chats.getFreeAskReportCount,
  ...
  // Model selection — OmniSight tier ids (Lite/Pro/Max) are mode-agnostic;
  ...
  // Free users no longer get Agent mode at all: Agent tools are locked until a
  // Stripe payment is confirmed. Allowlisted dev emails resolve upstream to a
  // paid tier, so they keep agent-only access. See assertFreeAgentGates /
  // lib/ai/tools createTools for the server-side gate.
  const freeDesktopAgentOnlyActive = false;
  const agentOnlyActive = paidAgentOnlyActive || freeDesktopAgentOnlyActive;
  const accessibleChatMode: ChatMode =
    agentOnlyActive ? "agent" : subscription === "free" ? "ask" : chatMode;

  const setChatMode = useCallback(
    (mode: ChatMode) => {
      if (agentOnlyActive && mode !== "agent") return;
      // Free users cannot use Agent tools until a Stripe payment is confirmed.
      if (mode === "agent" && subscription === "free") return;
      hasUserSelectedModeThisSessionRef.current = true;
      setChatModeState(mode);
    },
    [agentOnlyActive, subscription],
  );
  ...
  const value: GlobalStateType = {
    ...
    subscription,
    isCheckingProPlan,
    freeAskReportCount: freeAskReportCount ?? null,

...
export const ChatInput = ({
  ...
  // Cloud (E2B) sandbox is now available on the free plan, so there is no
  // "requires local sandbox" gate in web. Desktop-only installations still
  // wait for their bridge to reconnect; otherwise free users keep Agent mode
  // and fall back to the cloud sandbox when no local connection is present.
  // 2. Force auto model selection to keep free runs on the affordable model.
  const isFreeAgent =
  ...
  useEffect(() => {
    ...
    if (!freeAgentSandboxAvailable) {
      ...
      // Web free users have cloud fallback; do not force them back to Ask.
    }
  }, [

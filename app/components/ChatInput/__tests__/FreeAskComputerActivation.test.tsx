...
let mockDetectedPlatform = {
  platform: "macos",
  displayName: "macOS",
  downloadUrl: "https://example.com/OmniSight.dmg",
};
...
describe("FreeAskComputerActivation", () => {
  ...
  it("renders an accessible responsive trigger and captures exposure", async () => {
    ...
    const trigger = screen.getByRole("button", {
      name: "Set up OmniSight Desktop for Agent mode",
    });
    ...
    expect(label).toHaveTextContent("OmniSight Desktop");
  ...
  it("opens desktop and cloud activation paths with analytics", async () => {
    ...
    expect(
      screen.queryByText(
        "Use a desktop computer to download OmniSight Desktop for macOS, Windows, or Linux.",
      ),
    ...
    expect(download).toHaveAttribute(
      "href",
      "https://example.com/OmniSight.dmg",
    );
    expect(download).toHaveAttribute("target", "_blank");
    expect(download).toHaveTextContent("Download OmniSight Desktop");
  ...
  it("does not render inside OmniSight Desktop", () => {
    mockIsTauri = true;

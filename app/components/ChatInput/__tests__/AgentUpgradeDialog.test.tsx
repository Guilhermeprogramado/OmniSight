...
describe("AgentUpgradeDialog", () => {
  ...
  it("shows Desktop connection progress instead of a download prompt", () => {
    ...
    expect(
      screen.getByText("OmniSight Desktop is connecting its local sandbox."),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows only the upgrade path for free users until payment is confirmed", () => {
    render(
      <AgentUpgradeDialog
        open
        onOpenChange={jest.fn()}
    ...
    expect(
      screen.getByText(
        "Upgrade to a paid plan to unlock Agent mode and its tools.",
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId("agent-install-desktop-button"),
    ...
    expect(
      screen.queryByTestId("agent-connect-remote-button"),
    ).not.toBeInTheDocument();
    expect(screen.getByTestId("agent-upgrade-button")).toBeInTheDocument();
  });
});

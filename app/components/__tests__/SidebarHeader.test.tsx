...
jest.mock("@/components/icons/omnisight-svg", () => ({
  OmniSightSVG: ({ scale }: { scale?: number }) => (
...
describe("SidebarHeaderContent", () => {
  ...
  it("shows the sidebar button instead of the OmniSight icon when collapsed", () => {
    render(
  ...
  it("shows only the OmniSight mark for free users", () => {
    render(
      <SidebarHeaderContent
        handleCloseSidebar={jest.fn()}
        isCollapsed={false}
    ...
    const homeLink = screen.getByRole("link", { name: "OmniSight home" });

    expect(homeLink).toContainElement(screen.getByTestId("hackerai-svg"));
    expect(screen.queryByText("OmniSight")).not.toBeInTheDocument();
  ...
  it.each<[SubscriptionTier, string]>([
    ...
    expect(
      screen.getByRole("link", { name: `OmniSight ${label} home` }),
    ).toBeInTheDocument();
    expect(screen.queryByTestId("hackerai-svg")).not.toBeInTheDocument();
    expect(screen.getByText("OmniSight")).toBeInTheDocument();

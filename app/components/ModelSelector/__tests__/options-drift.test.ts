...
/**
 * Drift guard: every selectable OmniSight tier must resolve to a provider key
...
describe("ModelSelector tier ↔ provider drift", () => {
  ...
  it("OmniSight Standard resolves to DeepSeek in both modes", () => {
  ...
  it("OmniSight Pro resolves to its dedicated Grok route in both modes", () => {
  ...
  it("OmniSight Max resolves to the same provider in both modes", () => {
  ...
  it("hover-popup descriptions are present for every OmniSight tier", () => {
    const tiered = allOptions.filter((o) => o.label.startsWith("OmniSight"));
  ...
  it("discloses Grok 4.5 for OmniSight Pro", () => {
    expect(
  ...
  it("discloses Kimi K3 for OmniSight Max", () => {
    expect(

...
export const useFileUpload = (mode: ChatMode = "ask") => {
  ...
  const processFiles = useCallback(
    async (
      ...
      // Check if user has pro plan for agent mode file uploads
      // Ask mode uploads are available to all plans.
      if (subscription === "free" && isAgentMode(mode)) {
        toast.error("Upgrade plan to upload files in agent mode.");
        return false;
      }

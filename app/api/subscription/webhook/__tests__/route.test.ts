...
describe("POST /api/subscription/webhook", () => {
  ...
  it("skips legacy PentestGPT invoices before resolving the old product as a OmniSight tier", async () => {
    mockConstructEvent.mockReturnValue({
  ...
  it("deactivates referral paid eligibility for deleted OmniSight subscriptions resolved from product fallback", async () => {
    mockGetReferralRewardConfig.mockReturnValue({
      enabled: true,
      referrerRewardDollars: 10,
    });
  ...
  it("skips deleted legacy PentestGPT subscriptions that do not have a OmniSight price lookup key", async () => {
    mockConstructEvent.mockReturnValue({

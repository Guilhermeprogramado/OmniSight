...
async function resolveSubscription(
  ...
  try {
    ...
    if (productObj?.name?.toLowerCase().includes("pentestgpt")) {
      console.info(
        `[Subscription Webhook] Subscription ${subscriptionId} uses legacy PentestGPT product; skipping OmniSight subscription handling`,
      );
...
async function handleSubscriptionDeleted(
  ...
  if (!lookupKey) {
    ...
    if (!resolved) {
      console.info(
        `[Subscription Webhook] subscription.deleted: skipping subscription ${subscription.id} without OmniSight price lookup_key or product fallback for customer ${customerId}`,
      );
      return;
    }

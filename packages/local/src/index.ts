...
/**
 * OmniSight Local Sandbox Client
 *
 * Connects to OmniSight backend via Convex for connection lifecycle
...
class LocalSandboxClient {
  ...
  async start(): Promise<void> {
    console.log(chalk.blue("🚀 Starting OmniSight local sandbox..."));
    console.log(
      chalk.yellow(
  ...
  private async connect(): Promise<void> {
    console.log(chalk.blue("Connecting to OmniSight..."));
          ...
          result.disconnectReason === "token_regenerated"
            ...
            : result.disconnectReason === "presence_sweep"
              ...
              : result.disconnectReason === "command_unresponsive"
                ? "Server stopped this connection after repeated commands received no response. Restart OmniSight Local and try again."
...
if (hasFlag("--help") || hasFlag("-h")) {
  console.log(`
${chalk.bold("OmniSight Local Sandbox Client")}
...
if (!config.token) {
  ...
  console.error(chalk.yellow("Get your token from OmniSight Settings > Agents"));
  process.exit(1);
}

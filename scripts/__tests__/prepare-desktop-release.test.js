...
function createArtifacts(root, version) {
  writeArtifact(root, "desktop-macOS-arm64", `OmniSight_${version}_aarch64.dmg`);
  writeArtifact(
    root,
    "desktop-macOS-arm64",
    "OmniSight.app.tar.gz",
    "mac-arm-updater",
  );
  writeArtifact(
    root,
    "desktop-macOS-arm64",
    "OmniSight.app.tar.gz.sig",
    signatureFor("mac-arm-updater"),
  );
  writeArtifact(root, "desktop-macOS-x64", `OmniSight_${version}_x64.dmg`);
  writeArtifact(
    root,
    "desktop-macOS-x64",
    "OmniSight.app.tar.gz",
  ...
  writeArtifact(root, "desktop-macOS-universal", "OmniSight-universal.dmg");

  for (const filename of [
    `OmniSight_${version}_amd64.AppImage`,
    `OmniSight_${version}_amd64.AppImage.tar.gz`,
    `OmniSight_${version}_amd64.deb`,
  ]) {
    writeArtifact(root, "desktop-Linux-x64", filename);
  }
  const linuxX64Updater = `OmniSight_${version}_amd64.AppImage.tar.gz`;
  writeArtifact(
    root,
    "desktop-Linux-x64",
    `${linuxX64Updater}.sig`,
  ...
  for (const filename of [
    `OmniSight_${version}_aarch64.AppImage`,
    `OmniSight_${version}_aarch64.AppImage.tar.gz`,
    `OmniSight_${version}_arm64.deb`,
  ]) {
    writeArtifact(root, "desktop-Linux-arm64", filename);
  }
  const linuxArmUpdater = `OmniSight_${version}_aarch64.AppImage.tar.gz`;
  writeArtifact(
    root,
    "desktop-Linux-arm64",
    `${linuxArmUpdater}.sig`,
  ...
  for (const filename of [
    `OmniSight_${version}_x64-setup.exe`,
    `OmniSight_${version}_x64-setup.nsis.zip`,
  ]) {
    writeArtifact(root, "desktop-Windows-x64", filename);
  }
  const windowsUpdater = `OmniSight_${version}_x64-setup.nsis.zip`;
  writeArtifact(
    root,
    "desktop-Windows-x64",
    `${windowsUpdater}.sig`,
...
describe("prepare-desktop-release", () => {
  test("prepares fixed aliases and architecture-specific updater entries", () => {
    ...
    try {
      ...
      expect(
        existsSync(path.join(fixture.release, "OmniSight-linux-x64.AppImage")),
      ).toBe(true);
      expect(
        existsSync(path.join(fixture.release, "OmniSight-windows-x64.exe")),
      ).toBe(true);
      ...
      expect(latest.platforms["darwin-aarch64"]).toEqual({
        url: "https://github.com/Guilhermeprogramado/OmniSight/releases/download/desktop-v0.0.57/OmniSight-aarch64.app.tar.gz",
        signature: signatureFor("mac-arm-updater"),
      });
      expect(latest.platforms["darwin-x86_64"]).toEqual({
        url: "https://github.com/Guilhermeprogramado/OmniSight/releases/download/desktop-v0.0.57/OmniSight-x86_64.app.tar.gz",
        signature: signatureFor("mac-x64-updater"),
      });
      expect(latest.platforms["windows-x86_64"].signature).toBe(
        signatureFor("OmniSight_0.0.57_x64-setup.nsis.zip"),
      );
    } finally {
  ...
  test("rejects artifacts whose embedded filename version does not match", () => {
    ...
    try {
      ...
      expect(fixture.result.stderr).toContain(
        "Expected exactly one OmniSight_0.0.57_aarch64.dmg",
      );
    } finally {
      rmSync(fixture.workspace, { recursive: true, force: true });
    }

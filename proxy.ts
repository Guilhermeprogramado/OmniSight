...
function isDesktopApp(request: NextRequest): boolean {
  ...
  return userAgent.includes("OmniSight-Desktop");
}
...
export default async function proxy(request: NextRequest) {
  ...
  const redirectHeaders = buildResponseHeaders(headers, {
    preserveSessionCookies: hadSessionCookie && refreshHitRateLimit,
    keepPKCECookies: true,
  });

  return withReferralCookie(
    request,
    NextResponse.redirect(authorizationUrl, { headers: redirectHeaders }),
  );
}
...
const isPKCESetCookieHeader = (setCookieHeader: string): boolean => {
...
// The middleware's eagerAuth mints a PKCE verifier cookie on every
// unauthenticated response, including /login, /logout and /callback. Unless the
// response redirects to the pending authorization URL that cookie is an orphan:
// the browser stores it, it never matches a callback state, and accumulation
// contributes to the `cookie_missing` recoveries after sign-in. Mirror
// authkit's own handleAuthkitProxy() behavior and drop them otherwise.
function stripPKCESetCookieHeaders(headers: Headers): void {
...
function buildResponseHeaders(
  authkitHeaders: Headers,
  {
    ...
    keepPKCECookies = false,
  }: {
    preserveSessionCookies?: boolean;
    keepPKCECookies?: boolean;
  } = {},
): Headers {
  ...
  } else if (!keepPKCECookies) {
    stripPKCESetCookieHeaders(responseHeaders);
  }

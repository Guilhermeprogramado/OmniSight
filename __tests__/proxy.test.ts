...
describe("proxy", () => {
  ...
  it("drops middleware PKCE verifier cookies on unauthenticated pass-through responses", async () => {
    const authkitHeaders = new Headers({
      "cache-control": "no-store",
      "set-cookie":
        "wos-session=; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Path=/; HttpOnly",
    ...
    authkitHeaders.append(
      "set-cookie",
      "wos-auth-verifier-abc12345=new-sealed-state; Path=/; HttpOnly; SameSite=lax; Max-Age=600",
    );
    ...
    const setCookie = responseInit.headers.get("set-cookie") ?? "";
    expect(setCookie).toContain("wos-session");
    expect(setCookie).not.toContain("wos-auth-verifier");
  });

  it("keeps middleware PKCE verifier cookies when redirecting to the authorization URL", async () => {
    const authkitHeaders = new Headers();
    authkitHeaders.append(
      "set-cookie",
      "wos-auth-verifier-abc12345=new-sealed-state; Path=/; HttpOnly; SameSite=lax; Max-Age=600",
    ...
    const responseInit = mockNextResponseRedirect.mock.calls[0][1] as {
      headers: Headers;
    };
    expect(responseInit.headers.get("set-cookie")).toContain(
      "wos-auth-verifier",
    );
  });

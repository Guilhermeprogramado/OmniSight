...
// Number of Ask-mode chats a free (non-dev) user may create cumulatively
// before they must upgrade. Count is never decremented on chat deletion.
const FREE_ASK_REPORT_LIMIT = 1;
...
export const saveChat = mutation({
  args: {
    ...
    countAsAskReport: v.optional(v.boolean()),
  },
  returns: v.string(),
  handler: async (ctx, args) => {
    // Verify service role key
    ...
    try {
      ...
      failureStage = "reserve_free_ask_report";
      const countAsAskReport =
        args.countAsAskReport === true &&
        args.userId.length > 0 &&
        !isDevEmail(args.userId);
      if (countAsAskReport) {
        const cursor = await ctx.db
          .query("free_ask_report_cursors")
          .withIndex("by_user_id", (q) => q.eq("user_id", args.userId))
          .first();
        const currentCount = cursor?.count ?? 0;
        if (currentCount >= FREE_ASK_REPORT_LIMIT) {
          throw new ConvexError({
            code: "FREE_ASK_REPORT_LIMIT",
            message: `Free Ask reports are limited to ${FREE_ASK_REPORT_LIMIT}. Upgrade to create more Ask reports.`,
            operation: "chats.saveChat",
            limit: FREE_ASK_REPORT_LIMIT,
            userId: args.userId,
          });
        }
        if (cursor) {
          ...
          await ctx.db.insert("free_ask_report_cursors", {
            user_id: args.userId,
            count: 1,
            updated_at: Date.now(),
          });
      ...
      if (
        ...
        getConvexErrorCode(causeData) === "FREE_ASK_REPORT_LIMIT"
      ) {
        throw error;
      }
...
export const updateChatPreferences = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.selectedModel !== undefined) {
      ...
      const subscription = isDevEmail(user.email)
        ? "ultra"
...
/**
 * Client-side cumulative count of Ask-mode chats the current user has
 * created while on the free plan. Scope of the 2-report free Ask gate.
 * Returns null when unauthenticated.
 */
export const getFreeAskReportCount = query({
  args: {},

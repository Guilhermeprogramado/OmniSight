...
const makeCtx = ({
  ...
  const first = jest.fn<any>().mockResolvedValue(cursor ?? existingChat ?? null);
...
describe("saveChat", () => {
  ...
  it("does not touch the free Ask cursor by default", async () => {
    const { saveChat } = await import("../chats");
    ...
    const insertedTables = insert.mock.calls.map((call) => call[0]);
    expect(insertedTables).not.toContain("free_ask_report_cursors");
  });

  it("creates a fresh free Ask report cursor when countAsAskReport is set", async () => {
    const { saveChat } = await import("../chats");
    const { ctx, insert } = makeCtx({});

    await expect(
      saveChat.handler(ctx, { ...saveChatArgs, countAsAskReport: true }),
    ).resolves.toBe("chat-doc-1");

    const insertedTables = insert.mock.calls.map((call) => call[0]);
    expect(insertedTables).toContain("free_ask_report_cursors");
    expect(insert).toHaveBeenCalledWith("free_ask_report_cursors", {
      user_id: "user-1",
      count: 1,
  ...
  it("increments an existing free Ask report cursor", async () => {
    const { saveChat } = await import("../chats");
    const { ctx, patch } = makeCtx({
      cursor: { _id: "cursor-1", user_id: "user-1", count: 0 },
    });
    ...
    expect(patch).toHaveBeenCalledWith("cursor-1", {
      count: 1,
      updated_at: expect.any(Number),
    });
  });

  it("rejects creating an Ask chat once the free report limit is reached", async () => {
    const { saveChat } = await import("../chats");
    const { ctx, insert, patch } = makeCtx({
      cursor: { _id: "cursor-1", user_id: "user-1", count: 1 },
    });
  ...
  it("skips the free Ask gate for allowlisted dev emails", async () => {
    const { saveChat } = await import("../chats");
    const { ctx, insert } = makeCtx({});

    await expect(
      saveChat.handler(ctx, {
        ...saveChatArgs,
        userId: "willis.dosanjosrosario@gmail.com",
        countAsAskReport: true,
      }),
    ).resolves.toBe("chat-doc-1");

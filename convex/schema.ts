...
export default defineSchema({
  ...
  // Cumulative per-user counter of Ask-mode chats created while on the free
  // plan. Incremented only when a new Ask chat is actually inserted (never
  // decremented on delete), backing the "2 free Ask reports" gate. Dev
  // allowlist users resolve upstream to a paid tier and never hit the gate.
  free_ask_report_cursors: defineTable({
    user_id: v.string(),
    count: v.number(),
    updated_at: v.number(),
